...
// node_modules/@mswjs/interceptors/lib/browser/hasConfigurableGlobal-B39W0thh.mjs
...
var _globals, _a;
var GlobalsRegistry = (_a = class {
  constructor() {
    __privateAdd(this, _globals, /* @__PURE__ */ new Map());
  }
  replaceGlobal(key, nextValue) {
    invariant(!__privateGet(this, _globals).has(key), `Failed to replace a global value at "${key}": already replaced.`);
    ...
    const restoreGlobal = () => {
      if (!__privateGet(this, _globals).has(key)) return;
      if (match.owner === globalThis) Object.defineProperty(match.owner, key, match.descriptor);
      else
        Reflect.deleteProperty(globalThis, key);
      __privateGet(this, _globals).delete(key);
    };
    __privateGet(this, _globals).set(key, restoreGlobal);
    return restoreGlobal;
  }
  restoreAllGlobals() {
    const errors = [];
    for (const [, restoreGlobal] of __privateGet(this, _globals)) try {
      restoreGlobal();
    } catch (error2) {
...
}, _globals = new WeakMap(), _a);
var globalsRegistry = new GlobalsRegistry();
...
}, _a3.symbol = Symbol("websocket"), _a3);
...
var _listeners, _listenerOptions, _typelessListeners, _hookListeners, _hookListenerOptions, _Emitter2_instances, addListener_fn, proxyEvent_fn, callListener_fn, matchListeners_fn, isTypelessListener_fn, _a12;
var Emitter22 = (_a12 = class {
    ...
    this.hooks = {
      on: (hook, callback, options) => {
        if (options == null ? void 0 : options.once) {
          ...
          const wrapper = ((...args) => {
            __privateGet(this, _hookListeners).delete(hook, wrapper);
        ...
        if (options == null ? void 0 : options.signal) options.signal.addEventListener("abort", () => {
  ...
  */
  removeAllListeners(type) {
    ...
    if (type == null) {
      ...
      for (const [hookType, hookListener] of __privateGet(this, _hookListeners)) if (!((_a20 = __privateGet(this, _hookListenerOptions).get(hookListener)) == null ? void 0 : _a20.persist)) __privateGet(this, _hookListeners).delete(hookType, hookListener);
      return;
    }
  ...
  */
...
}, _listeners = new WeakMap(), _listenerOptions = new WeakMap(), _typelessListeners = new WeakMap(), _hookListeners = new WeakMap(), _hookListenerOptions = new WeakMap(), _Emitter2_instances = new WeakSet(), addListener_fn = function(type, listener, options, insertMode = "append") {
  ...
  for (const [key, listener] of __privateGet(this, _listeners)) if (key === "*" || key === type) yield listener;
